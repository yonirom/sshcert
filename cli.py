...
def password_hash(password: str) -> str:
    salt = os.urandom(16)
    hashed_password = hashlib.scrypt(password.encode('utf-8'), salt=salt, n=2**14, r=2, p=1)
...
@click.pass_context
def create(ctx, username, password, password_stdin, publickey, principals, extensions, valid_for):
    ...
    if password and password_stdin:
        click.echo("Error: --password and --password-stdin are mutually exclusive.", err=True)
        return
...
@click.pass_context
def update(ctx, username, password, password_stdin, publickey, principals, extensions, valid_for):
