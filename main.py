    ...
    async def validate_public_key(self, username, key):
        ...
        if self.user_found:
            try:
                ...
                return False
            if key == user_key:
                logger.info(f"publickey authentication succeeded for {username}")
                return True
        return False
...
def sign_certificate(
...
) -> Tuple[SSHKey, SSHOpenSSHCertificate]:
    ...
    user_certificate: SSHOpenSSHCertificate = ca.generate_user_certificate(
        new_user_key,
        key_id="meepmeep",
        principals=principals,
        valid_after=datetime.now(),
        valid_before=user_config.valid_for,
    )
...
class EntryPoint:
    ...
    async def monitor_conf_dir(self, directory: Path):
        async for _ in awatch(
            directory, watch_filter=ConfigFilter(), recursive=False, force_polling=False
        ):
...
class ConfigFilter(DefaultFilter):
    def __call__(self, change: Change, path: str) -> bool:
        return (
            ...
            and path.endswith("config.yaml")
            and change in (Change.added, Change.modified)
        )
